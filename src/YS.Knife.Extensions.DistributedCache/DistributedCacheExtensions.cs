...
namespace Microsoft.Extensions.Caching.Distributed
{
    public static class DistributedCacheExtensions
    {
        ...
        public static async Task<T> GetOrAddObjectAsync<T>(this IDistributedCache cache, string key, Func<Task<T>> valueFactory, DateTimeOffset absoluteExpiration, bool cacheDefaultValue = false, JsonSerializerOptions options = default)
        {
            var content = await cache.GetStringAsync(key);
            if (string.IsNullOrEmpty(content))
            {
                ...
                {
                    var body = val.ToJsonText(options);
                    await cache.SetStringAsync(key, body, new DistributedCacheEntryOptions { AbsoluteExpiration = absoluteExpiration });
                }
                return val;
            }
            else
        ...
        {
            return cache.GetOrAddObjectAsync(key, () => Task.FromResult(value), absoluteExpiration, cacheDefaultValue, options);
        }
